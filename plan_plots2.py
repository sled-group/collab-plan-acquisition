...
def print_epoch(data,lst,exp):
    ...
    for bin_id in range(len(bins)):
        ...
        out_bins[bin_id] = np.std(f1), np.mean(acc), np.mean(f1), np.mean(iou), predicts, targets, [x for x in iou], np.std(iou), np.mean(iou)
...
def do_split(model, load_str,lst,exp,baseline=None,baseline2=None,global_plan=False, player_plan=False, incremental=False):
    ...
    for load_path in load_str:
        ...
        for game_num, game in enumerate(lst):
            ...
            # print(prediction.shape)
            # exit(0)
            data += list(zip(prediction.cpu().data.numpy(), [ground_truth.cpu().data.numpy()]*len(prediction),[game_num]*len(prediction),[seed_num]*len(prediction),list(range(len(prediction))),[len(prediction)]*len(prediction) ))
    ...
    list_str_fun = lambda lst: '['+', '.join([f'{x:5.3f}' for x in lst[:200]])+']'
    
    f1_std, acc, f1, iou, predicts, targets, acc2, iou_std, iou_avg = print_epoch(data,lst,exp)
