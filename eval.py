...
for i,ttl in enumerate(['Completed Task Status', 'Other Player\'s Knowledge', 'Other Player\'s Current Task']):
    for j,hdata in enumerate([hdata1]):
        ...
        c1 = [2.576*s/np.sqrt(l) for s,l in zip(s1,l1)]
        c2 = [2.576*s/np.sqrt(l) for s,l in zip(s2,l2)]
        width = 2/5
        ax1.bar(bins-width/2, x1, width, color='blue', yerr=c1, capsize=7, alpha=0.5, label='Matches',linewidth=2.0)#, marker='x')
        ...
        # print(x2)
        width = 2/5
        ax1.bar(bins-width/2, x1, width, color='blue', yerr=c1, capsize=7, alpha=0.5, label='Before Question',linewidth=2.0)#, marker='x')
        ...
        c1 = [2.576*s/np.sqrt(l) for s,l in zip(s1,l1)]
        c2 = [2.576*s/np.sqrt(l) for s,l in zip(s2,l2)]
        c3 = [2.576*s/np.sqrt(l) for s,l in zip(s3,l3)]
        c4 = [2.576*s/np.sqrt(l) for s,l in zip(s4,l4)]
