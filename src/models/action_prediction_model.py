...
class Model(nn.Module):
    def __init__(self, seq_model_type=0):
        ...
        dlist_hidden = 1024
        frame_emb = 512
        drnn_in = 1024 + 2 + frame_emb # + q_emb
        # drnn_in = 1024 + 2
        ...
        self.action = qlayer(q_in_size,21)
        self.player = qlayer(q_in_size,2)
    
    def forward(self,game,global_plan=False, player_plan=False,pov=None,exp=0):
        retval = []
        
        l = list(game)
        # print(l[0])
        _,d,dl,q,f,al,*_ = zip(*list(game))
        ...
        #     return []
        # d,q = zip(*flt_lst)
        d = [np.concatenate(([int(x[0][1]-pov),int(not x[0][1]==pov)],x[0][-1])) if not x is None else np.zeros(1026) for x in d]
        
        # dl = [x if not x is None else [(0,pov,19)] for x in dl]
        
        # dl = [x if x[0][1]==pov else [(0,pov,19)] for x in dl]
        
        # dl = dl[1:] + [[(0,pov,19)]]
        
        # dl = [None if x[0][2] == 19 else x for x in dl]
        
        
        dl = list(dl[1:]) + [None]
        if exp==1:
            dl = [[(x[0][0],x[0][1],15)] if not x is None else None for x in dl]
        dl = [x if not x is None and x[0][1]==pov else None for x in dl]
        idxs = [i for i,x in enumerate(dl) if not x is None]
        # idxs = idxs[1:3:]
        idxs = idxs[0:3:] + idxs[2:3:]#idxs[1:3:]#
        for idx in idxs:
            for jdx in range(idx - 10, idx):
                if (jdx > 0) and dl[jdx] is None :
                    dl[jdx] = [(0,pov,19)]
                    break
        for idx in idxs:
            for jdx in range(idx - 20, idx):
                if (jdx > 0) and dl[jdx] is None :
                    dl[jdx] = [(0,pov,19)]
                    break
        # for x in dl:
        ...
        fun_lst = [self.action, self.player]
        fun = lambda x: [f(x) for f in fun_lst]
   
    
        retval = [(_l,fun(torch.cat((plan_emb,torch.tensor(_q).float().to(DEVICE),_y)))) for _y, _q, _l in zip(y,q,al)if not _l is None]
        return retval