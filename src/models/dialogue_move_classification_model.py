...
class Model(nn.Module):
    def __init__(self, seq_model_type=0):
        ...
        if seq_model_type==0:
            ...
            mask_fun = lambda x: torch.tril(torch.ones(x.shape[0],x.shape[0]),diagonal=1).bool().to(DEVICE)
        ...
        slot_out_dim = 1+12+6+21
        
        self.d_act = qlayer(q_in_size,35)
        self.d_slot1 = qlayer(q_in_size,slot_out_dim)
        self.d_slot2 = qlayer(q_in_size,slot_out_dim)
        self.d_slot3 = qlayer(q_in_size,slot_out_dim)
    
    def forward(self,game,global_plan=False, player_plan=False, intermediate=False):
        retval = []
        
        l = list(game)
        _,d,dl,q,f,_,_,m = zip(*list(game))
        # print(m)
        # exit()
        ...
        u = torch.cat((
            torch.tensor(np.array(d)).float().to(DEVICE),
            torch.tensor(np.array(q)).float().to(DEVICE),
            self.conv(torch.tensor(np.array(f)).permute(0,3,1,2).float().to(DEVICE)).reshape(-1,512)
            ),axis=-1)
        u = u.float().to(DEVICE)
        ...
        fun_lst = [self.d_act, self.d_slot1, self.d_slot2, self.d_slot3]
        fun = lambda x: [f(x) for f in fun_lst]
   
    
        retval = [(_l,fun(torch.cat((plan_emb,torch.tensor(_q).float().to(DEVICE),_y)))) for _y, _q, _l in zip(y,q,m)if not _l is None]
        return retval