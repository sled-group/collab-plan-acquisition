...
#     for exp in [0,1,2]:
                ...
                for pov in point_of_view:
                    ...
                    try:
                        ...
                        if 'Yes' in files[0]:
                            ...
                            print('  V & ', end=' ')
                        print(f'{files[0].split("_")[-2]} & {np.mean(f):0.3f}({2.576*np.std(f)/np.sqrt(len(f)):0.3f})', end=' ')
                        print(f' {np.mean(fv):0.3f}({2.576*np.std(fv)/np.sqrt(len(fv)):0.3f}) \\\\')
        ...
        for idx,(g,p,a,f) in enumerate([(g1,p1,a1,f1),(g2,p2,a2,f2)]):
            ...
            means = [np.mean(x) for x in a]
            for i,(m,s) in enumerate(zip(means,stds)):
                ax1.plot([i+1,i+1],[m-2.576*s/np.sqrt(10),m+2.576*s/np.sqrt(10)],linewidth=1,color='black')
                ax1.plot([i+0.75,i+1.25],[m-2.576*s/np.sqrt(10),m-2.576*s/np.sqrt(10)],linewidth=1,color='black')
                ax1.plot([i+0.75,i+1.25],[m+2.576*s/np.sqrt(10),m+2.576*s/np.sqrt(10)],linewidth=1,color='black')
            if not human is None:
                ax1.plot([0.5,6.5],[human]*2)
            ax1.plot([0.5,6.5],[rnd]*2,color='orange')
