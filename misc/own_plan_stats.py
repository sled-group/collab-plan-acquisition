                ...
                if idx in possible_cand:
                    ...
                    for jdx, aaa in enumerate(aa):
                        if idx in possible_mats:
                            ...
                            predicts.append(a[idx,jdx])
                            targets.append(b.reshape(21,21)[idx,jdx])
                        else:
                            a[idx,jdx] = 0
                else:
                    for jdx, aaa in enumerate(aa):
...
def do_split(model,lst,exp,baseline=None,baseline2=None,global_plan=False, player_plan=False):
    data = []
    acc_loss = 0
    p = []
    g = []
    for batch, game in enumerate(lst):
        ...
        _, y = model(game, global_plan=global_plan, player_plan=player_plan)
    ...
    if not baseline2 is None:
        ...
        print(f'{f1_avg:5.3f} & ',f'        ',end=' ')
        # exit()
    print('\\\\',end=' ')
    
    
    # exit()
    return acc0, acc_loss, data, acc, f1, iou, predicts, acc2
...
def main(args):
    ...
    # print('Test')
    args.intermediate = 0
    load_str = f'{args.save_path[:-7]}{args.intermediate}.torch'
    # print(load_str,end='\t')
    for _ in range(4):
        print('  & ',end='\t')
    model.load_state_dict(torch.load(f'{args.save_path[:-7]}{args.intermediate}.torch'))
    ...
    train = None
    if args.pov=='None':
        test = [GameParser(f,d_flag,0,args.intermediate) for f in dataset_splits['test']]
        if args.experiment > 2:
            test += [GameParser(f,d_flag,4,args.intermediate) for f in dataset_splits['test']]
    elif args.pov=='Third':
        test = [GameParser(f,d_flag,3,args.intermediate) for f in dataset_splits['test']]
    elif args.pov=='First':
        test  = [GameParser(f,d_flag,1,args.intermediate) for f in dataset_splits['test']]
        test += [GameParser(f,d_flag,2,args.intermediate) for f in dataset_splits['test']]
    else:
        print('POV must be in [None, First, Third], but got', args.pov)
    model.eval()
    acc, acc_loss, data, _, f1, iou, predicts, acc2 = do_split(model,test,args.experiment,global_plan=global_plan, player_plan=player_plan)
    print()
    
    
    for args.intermediate in [1,2,4,8]:
        if args.intermediate < 16:
            load_str = f'{args.save_path[:-7]}{args.intermediate}.torch'
            # print(load_str,end='\t')
            model.load_state_dict(torch.load(load_str))
        else:
            load_str = f'models/upperbound/plan_exp2_LSTM_int{args.intermediate}.torch'
            # print(load_str,end='\t')
            model.load_state_dict(torch.load(load_str))
        ...
        if args.pov=='None':
            ...
            print('POV must be in [None, First, Third], but got', args.pov)
        model.eval()
        acc, acc_loss, data, _, f1, iou, _, _ = do_split(model,test,args.experiment,baseline=predicts,baseline2=acc2,global_plan=global_plan, player_plan=player_plan)
        print()
    exit(0)
